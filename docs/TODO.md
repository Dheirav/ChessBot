...
| **6.2 remainder: the general tune** | — | **closed 2026-09-01, `BUGS.md` 20** — three attempts, all cancelled by node cost | ~~ Texel-style, over an evaluation whose largest term has stopped shouting. Position set exists: the game archive plus `evalref`'s 23 603 positions. |
...
| **Sweep the razoring margin** | — | **done 2026-08-28, null** | 350 against the shipped 500 measured **−1.0 [−10.0, +7.9]** over 3 360 games, `shard-20260828-002812/`. 3.1's 57-Elo swing was against **200cp**, inside the evaluation's own measured error; the curve between 350 and 500 is flat. 500 stays. 700 is untested but the flatness argues against it mattering. |
